...
var dcosineDistance = tryRequire( resolve( __dirname, './../lib/dcosine_distance.native.js' ) );
var opts = {
	'skip': ( dcosineDistance instanceof Error )
};
...
tape( 'the function has an arity of 5', opts, function test( t ) {
	t.strictEqual( dcosineDistance.length, 5, 'returns expected value' );
	t.end();
});

tape( 'the function calculates the cosine distance between two strided arrays', opts, function test( t ) {
	var c;
	var x;
	var y;
	...
	c = dcosineDistance( x.length, x, 1, y, 1 );
...
tape( 'the function supports complex access patterns', opts, function test( t ) {
	...
	// Test against textbook formula: 1-(((1*8)+(3*7)+(5*6))/(5.916*12.206))
	c = dcosineDistance( 3, x, 2, y, -1 );
	t.strictEqual( isAlmostSameValue( c, 0.1829947349814245, 1 ), true, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports view offsets', opts, function test( t ) {
	...
	// Test against textbook formula: 1-(((2*9)+(4*10)+(6*11))/(7.483*17.378))
	c = dcosineDistance( 3, x1, 2, y1, 1 );
	t.strictEqual( isAlmostSameValue( c, 0.04649230952265904, 1 ), true, 'returns expected value' );
	t.end();
});
